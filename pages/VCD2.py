...
def create_gnuplot_package(data_list, style_dict, x_lim, vcd_lim, ir_lim):
    ...
    for i, d in enumerate(data_list):
        ...
        # Gnuplot: Column order is Wavenumber, IR, VCD
        # Note: plot_cmds logic needs to match data structure
...
def main():
    ...
    with tab2:
        ...
        with col_c_opt:
            ...
            show_legend = c_leg.checkbox("凡例を表示", value=True)
            
            with st.expander("軸範囲の設定", expanded=False):
                st.markdown("###### X軸 (波数)")
                c1, c2 = st.columns(2)
                t2_x_high = c1.number_input("X Left", value=2000.0, key="t2_xh")
                t2_x_low = c2.number_input("X Right", value=800.0, key="t2_xl")
                
                st.markdown("---")
                st.markdown("###### Y軸 (強度)")
                man_t2 = st.checkbox("Y軸の範囲を固定する", key="t2_man")
                
                t2_vcd_min, t2_vcd_max = None, None
                t2_ir_min, t2_ir_max = None, None
                
                if man_t2:
                    st.caption("VCD Range")
                    c3, c4 = st.columns(2)
                    t2_vcd_max = c3.number_input("VCD Max", value=0.0005, format="%.5f", step=0.0001, key="t2_vmax")
                    t2_vcd_min = c4.number_input("VCD Min", value=-0.0005, format="%.5f", step=0.0001, key="t2_vmin")
                    
                    st.caption("IR Range")
                    c5, c6 = st.columns(2)
                    t2_ir_max = c5.number_input("IR Max", value=1.0, step=0.1, key="t2_imax")
                    t2_ir_min = c6.number_input("IR Min", value=0.0, step=0.1, key="t2_imin")
        ...
        if target_data:
            with st.expander("設定パネル", expanded=True):
                ...
                for i, d in enumerate(target_data):
                    ...
                    with cols[i % 3]:
                        ...
                        p_color = cc.color_picker("Color", value=default_c, key=f"c_{i}")
                        p_width = cw.number_input("Wid", value=1.5, step=0.5, key=f"w_{i}")
                        p_scale = cs.number_input("Scl(x)", value=1.0, step=0.5, key=f"s_{i}")
            ...
            if man_t2: 
                ax_vcd.set_ylim(t2_vcd_min, t2_vcd_max)
            ...
            if man_t2: 
                ax_ir.set_ylim(t2_ir_min, t2_ir_max)
            ...
            buf.seek(0)
            c1.download_button("画像保存 (PNG)", buf, "comparison_plot.png", "image/png")
            ...
            if zip_dat:
                c2.download_button("Gnuplotデータ (.zip)", zip_dat, "gnuplot_data.zip", "application/zip")
