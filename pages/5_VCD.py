            ...
            with st.form("tab4_form"):
                with st.expander("🎚️ パラメータ & 🎨 スタイル設定", expanded=True):
                    ...
                    with col_para2:
                        ...
                        # 0.00001 (1e-5) まで設定可能に変更
                        scale_int_vcd = st.number_input("VCD Scale", value=1.0, step=0.00001, format="%.5f")
                        scale_int_ir = st.number_input("IR Scale", value=1.0, step=0.00001, format="%.5f")
                    with col_para3:
                        st.markdown("**表示設定**")
